...
class QueryBuilder:
    ...
    async def find(self, id):
        ...
        return self.type(response[self.type.RESOURCE[:-1]])

    async def find_many(self, url, type, resource):
        ...
        """
        list = []
        response = await RestClient.get(url)[resource]
        if len(response) > 0:
    ...
    async def all(self):
        ...
        """
        list = [x async for x in self.iter()]
    ...
    async def iter(self):
        ...
        while True:
            response = await RestClient.get(url, self.params)
            response = response[self.type.RESOURCE]
            if len(response) > 0:
                for item in response:
    ...
    async def array(self):
        ...
        url = "{}/{}".format(__endpoint__, self.type.RESOURCE)
        response = await RestClient.get(url, self.params)
        return response[self.type.RESOURCE]
